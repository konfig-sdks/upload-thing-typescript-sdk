...
import { readableStreamToString, UploadThingError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new UploadThingError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new UploadThingError(e, e.response?.data)
                }
                if (innerError instanceof UploadThingError) {
                    // Got "UploadThingError" from the above try block
                    throw innerError;
                }
